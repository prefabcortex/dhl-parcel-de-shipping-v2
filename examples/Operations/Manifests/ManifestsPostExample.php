...
final class ManifestsPostExample
{
     ...
     * parameter. ''profile'' attribute (request body parameter) - defines the user group profile. A
     * user group is permitted to specific billing numbers. Shipments are only closed out if they
     * belong to a billing number that the user group profile is entitled to use. This attribute is
     * mandatory. Please use the standard user group profile ''STANDARD_GRUPPENPROFIL'' if no
     * dedicated user group profile is available. ''billingNumber'' attribute (query parameter) -
     * defines the billing number for which shipments shall be closed out. If a billing number is
     * set, then only the shipments of that billing number are closed out. In that case no list of
     * specific shipment numbers needs to be passed. ''shipmentNumbers'' attribute (request body
     * parameter) - lists the specific shipping numbers of the shipments that shall be closed out.
     * If all shipments shall be closed, the query parameter ''all'' needs to be set to ''true''. In
     * that case neither a billing number nor a list of shipment numbers need to be passed in the
     * request.
     *
     * #### Response
     *
    ...
    /**
     * NOTE: This is a structurally valid skeleton generated from the OpenAPI schema (required
     * properties, plus optional properties that carry a spec `example:`). oneOf/anyOf alternatives
     * always pick the first option. It does NOT encode business rules the spec only documents in
     * prose (e.g. "either X or Y must be set"). Review the values and the API documentation before
     * production use.
     */
