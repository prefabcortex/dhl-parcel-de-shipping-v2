 ...
 *
 * Two corruptions of a valid document, both derived from the API description: a required
 * property removed, and a property given a value of a type it cannot hold. Each has to
 * raise `MalformedDataException` — the class the operations already name in
 * their `@throws`, so a caller following the documented contract catches it.
 *
 * The alternative is what makes this worth asserting: an unchecked value reaching a typed
 * constructor raises `TypeError`, which is an `Error` rather than an `Exception` and which
 * no `@throws` here mentions. Both look identical until something runs.
 *
 * What this cannot show is that the service sends what its description promises. These
 * documents were built from that description too.
 */
final class MalformedDataTest extends TestCase
{
    ...
    public function testAMissingRequiredPropertyIsRejected(SelfNormalizingModel $model, callable $fromArray, array $requiredProperties): void
    {
    ...
    public function testAMistypedPropertyIsRejected(SelfNormalizingModel $model, callable $fromArray, array $wrongValues): void
    {
